...
public class Percolation {
    private final int size; // number of sites in each row or column
    private boolean[] opened; // value is true if opened
    private int openedCount; // number of open sites
    private final int virtualTop;  // check percolation by seeing of virtualTop and bottom are connected
    ...
    public void open(int row, int col) {
        ...
        if (row == 1) {
            wQU.union(oneD, virtualTop);
        }
        if (row == size) {
            wQU.union(oneD, virtualBottom);
        }
    }
    ...
    public boolean isFull(int row, int col) {
        ...
        int oneD = xyTo1D(row, col);
        return (wQU.connected(oneD, virtualTop));  
    }
